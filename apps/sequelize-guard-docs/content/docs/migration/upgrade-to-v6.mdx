...
description: "Complete guide for upgrading to sequelize-guard v6.x"
---


## Overview

Version 6.0 is the first major release of sequelize-guard with the following key improvements:

- **TypeScript Rewrite**: Full TypeScript support with complete type definitions
- **Sequelize 6 Compatibility**: Built to work with Sequelize v6.x
- **Modern Tooling**: Built with Vite, tested with Vitest
- **Better Type Safety**: Enhanced IntelliSense and compile-time type checking
- **ESM & CommonJS**: Dual package support for modern and legacy projects

> **ℹ️ Note:** If you're upgrading from an older version (pre-v6), the main effort is upgrading to Sequelize 6 first.
...
```json tab="v6.x (Current)"
"engines": {
  "node": ">=20.19.0"
}
```

```json tab="Older versions"
"engines": {
  "node": ">=12.0.0"
}
```
...
- **Older versions:** Used Sequelize ^5.x
...
- **Older versions:** No official TypeScript definitions
...
If you're migrating from Sequelize 5 to 6, review the [Sequelize v6 migration guide](https://sequelize.org/docs/v6/other-topics/upgrade/).

Key Sequelize 6 changes:

```js tab="Sequelize 6 (Current)"
