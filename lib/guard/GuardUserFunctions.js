...
module.exports = (guard) => {
  ...
  /**
   @description remove previous roles and assign new set.

   @example

   user.fixRoles(['admin', 'author']);

   * @param  {Array<string>} roles - Roles to replace
   */
  GuardUser.prototype.fixRoles = function (roles) {
    return this.roles()
      .then((previousRoles) => this.rmAssignedRoles(previousRoles)
        .then(() => this.assignRoles(roles)
          .then(() => this.roles())
        )
      );
  };
  ...
  /**
   * Get user privileges on resource to view or modify fields.
   *
   @example

   user.getPrivileges('blog');

   * @param  {string|undefined|null} resource - Resource to get privileges
   *
   * @returns {Array} - list of privileges
   */
  GuardUser.prototype.getPrivileges = function (resource) {
    return guard.userPrivileges(this, resource);
  };
};
