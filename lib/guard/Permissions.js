...
module.exports = (SequelizeGuard) => {
  ...
  SequelizeGuard.prototype.createPerms = function (
    ...
    let permissions = this._sanitizePermsInput(resources, actions, fields, options);
   ...
   *      {resource : "blog", action: ["view","edit"], fields: ["title", "body"], name: "blog_view_edit"]},
   *      {resource : "post", action: ["view"], fields: ["title"]},
   *      {resource : "image", action: "view", fields: "alt"},
   * ])
   *
   *
  ...
  SequelizeGuard.prototype.createPermsBulk = function (
    ...
    permissions = permissions.map((p, i) => {
      ...
      if (typeof p.fields === 'string') _fields = [p.fields];
      if (Array.isArray(p.fields)) _fields = p.fields;
   ...
   *
   *  guard.findPerms({resource : 'blog', action : 'view', fields: 'title'}).then((perms) =>{});
   *
   ...
   * @param  {string} args.fields - nae of field eg. name, description
  ...
  SequelizeGuard.prototype.findPerms = async function (args = {}) {
    ...
    if (args.search) {
      ...
      if (args.fields) {
        wheres.push({ fields: { [Op.substring]: args.fields } });
      }
    } else {
      ...
      if (args.fields) wheres.push({ fields: args.fields });
    }
