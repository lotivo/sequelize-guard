...
module.exports = {
  up: async (queryInterface, Sequelize, opts) => {
    ...
    var p1 = queryInterface.bulkInsert(`${prefix}roles`, roles2insert);
    var p2 = queryInterface.bulkInsert(`${prefix}permissions`, perms);

    return Promise.all([p1, p2]).catch((e) => console.log(e));
  },
  ...
  down: (queryInterface, Sequelize, opts) => {
    ...
    var p1 = queryInterface.bulkDelete(
      `${prefix}roles`,
      roles.map((r) => ({ name: r }))
    );

    var p2 = queryInterface.bulkDelete(
      `${prefix}permissions`,
      perms.map((r) => ({ name: r.name }))
    );
    var p2 = queryInterface.bulkInsert(`${prefix}permissions`, perms);
