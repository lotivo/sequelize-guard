...
module.exports = (SequelizeAcl) => {
   ...
   *
   */
  SequelizeAcl.prototype.createPerms = function (
    resources,
    actions,
    options = {}
  ) {
   ...
   *
   */
  SequelizeAcl.prototype.createPermsBulk = function (
    permissions,
    options = {}
  ) {
    let _acts = [];
   ...
   * @example
   *
   *  acl.findPerms().then((perms) =>{});
   *
   *  acl.findPerms({name : 'admin'}).then((perms) =>{});
   *
   *  acl.findPerms({resource : 'blog', action : 'view'}).then((perms) =>{});
   *
   *  acl.findPerms({action : 'view', search : true}).then((perms) =>{});
   *
   ...
   */
  SequelizeAcl.prototype.findPerms = async function (args = {}) {
    let acl = this;
    let wheres = [];
    let cond = { where: {} };
  ...
  SequelizeAcl.prototype._sanitizePermsInput = function (
    resources,
    actions,
    options = {}
  ) {
